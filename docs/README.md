...
- 成果や意識した点
  - IAM の過剰な権限付与を防ぐために、最小権限で IAM ロールやユーザーを作成し、権限エラーとなった場合に開発者は申告制でリソースは構築時に権限を追加する方法を取り、開発速度を犠牲に最小権限の原則に従って、クラウドのセキュリティ向上の方針でインフラ構築を進めたこと
...
- 苦労した点
  - イーサリアム決済機能の実装が困難で自分の知識不足や社内に相談できる方が不在のため、ライブラリの公式ドキュメントや GitHub から情報を得て、自己解決するまで工数を要したこと
...
- 担当業務
  ...
  - 認証機能の実装（jwt-auth)
- 成果や意識した点
  - バックエンド開発経験者が少なかったため、メンバーのキャッチアップ速度を上げるために雛形となるコードの実装やコードレビューや実装方針の相談に乗り、チーム全体がスキルアップして開発速度を上げる取り組みを行ない、遅延なくプロジェクトを進めることに貢献したこと
  - ReactNative 製アプリの画面遷移時の UX 向上を目的とし、ナビゲーションライブラリの特徴を活かすためにフロントチームと API のスキーマについて調整を行なった結果、アプリ起動時の速度を犠牲にスムーズな画面遷移をするアプリ開発を意識したこと
